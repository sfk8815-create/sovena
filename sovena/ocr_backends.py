...
后端选择（环境变量）：
  SOVENA_OCR_BACKEND = mlx | http | auto（默认 auto：设置了
  SOVENA_OCR_API 就用 http，否则 mlx）

http 后端相关变量：
  SOVENA_OCR_API        服务地址（如 http://localhost:8080/v1）
  SOVENA_OCR_MODEL_NAME 模型名（默认 Unlimited-OCR）
  SOVENA_OCR_API_KEY    可选 bearer key
"""
...
DEFAULT_OCR_API = os.environ.get("SOVENA_OCR_API", "")
DEFAULT_OCR_MODEL_NAME = os.environ.get("SOVENA_OCR_MODEL_NAME", "Unlimited-OCR")
DEFAULT_OCR_API_KEY = os.environ.get("SOVENA_OCR_API_KEY", "")
...
def resolve_backend() -> str:
    """决定 OCR 后端：显式 SOVENA_OCR_BACKEND 优先，否则按是否配置 API 自动。"""
    choice = (os.environ.get("SOVENA_OCR_BACKEND") or "auto").lower()
...
def backend_info() -> dict:
    ...
    return {
        ...
        "mlx_model_dir": os.path.expanduser(
            os.environ.get("SOVENA_OCR_MODEL", "~/models/Unlimited-OCR-MLX")
        )
        if backend == "mlx"
        else None,
    }
