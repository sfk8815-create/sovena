"""缙云文采（Sovena）MCP 服务：把文献流能力以 MCP 工具形式暴露给 AI 客户端。

与 Web UI 同进程同端口：
    python -m sovena.server   （或 uv run sovena serve）
...
工具一览：
    ...
    ---- sovena 核心 ----
    sovena_prepare          一键「将子分类做好 AI 调用准备」（默认增量）
    sovena_job_status       任务进度/日志
    sovena_jobs             任务列表
    sovena_cancel_job       取消任务
    sovena_search           自然语言语义检索（跨语言、带原书页码）
    sovena_find_similar     按条目找语义相似文献
    sovena_manifest         分类文献包清单
    sovena_read_item        读取文献 markdown 全文
    ---- adhoc 临时资料 ----
    sovena_adhoc_process    任意文件/文件夹 → OCR+向量索引
    sovena_adhoc_list       已有资料包列表
    ---- 运维 ----
    sovena_doctor           连通性诊断（Zotero/embedding/目录权限/OCR）
    sovena_system_status    系统资源状态
...
mcp = FastMCP(
    "sovena",
    instructions=(
        "缙云文采（Sovena）：学科学术文献流服务（人文/社科/理工/医学等通用）。工作流程：① zotero_collections / zotero_search "
        "找到 Zotero 分类或条目；② sovena_prepare 把分类转换为语义文献包并建向量索引"
        "（默认增量，大分类耗时长，可传 wait=False 后台执行并用 sovena_job_status 轮询）；"
        "③ sovena_search 做自然语言检索（支持跨语言，命中带原书页码 page 字段）；"
        "④ sovena_read_item 读取全文。非 Zotero 的本地资料（任意文件夹/文件）用 "
        "sovena_adhoc_process 处理后同样可检索。引用时注意 page 是原书页码非物理页序。"
    ),
)
...
DEFAULT_HOST = os.environ.get("SOVENA_HOST", "0.0.0.0")   # 0.0.0.0 供 tailscale 远程访问
DEFAULT_PORT = int(os.environ.get("SOVENA_PORT", "8765"))
...
def zotero_search(
    ...
    zotero_item 看详情，或用 sovena_read_item 读已准备的全文。
    """
...
def sovena_prepare(
    ...
    if not wait:
        return f"任务已提交：{job.id}。用 sovena_job_status 查询进度。"
    ...
    return (
        ...
        f"{mgr.get(job.id).status}）。用 sovena_job_status 继续查询。"
    )


@mcp.tool
def sovena_job_status(job_id: str) -> str:
    """查询任务进度与最近日志。"""
...
@mcp.tool
def sovena_cancel_job(job_id: str) -> str:
    """取消一个排队/运行中的任务。"""
...
@mcp.tool
def sovena_search(query: str, collection: str | None = None, top_k: int = 8) -> str:
    ...
    if not hits:
        return "无结果。请确认该分类已用 sovena_prepare 准备。"
    out = []
    for h in hits:
...
@mcp.tool
def sovena_find_similar(item_key: str, top_k: int = 5) -> str:
...
@mcp.tool
def sovena_manifest(collection: str) -> str:
...
@mcp.tool
def sovena_read_item(collection: str, title: str) -> str:
    """按题名（或题名片段/条目目录名）读取某条文献的 markdown 全文。

    全文可能很长；建议先 sovena_search 定位，再读取命中文献。
    """
    mf = _packager.load_manifest(collection)
    if mf is None:
        return f"分类「{collection}」尚未准备"
...
def sovena_adhoc_process(
    ...
    while time.time() < deadline:
        ...
        if j and j.status in ("done", "error", "cancelled"):
            return _fmt_job(j)
        time.sleep(2)
    return f"等待超时（任务 {job.id} 仍在后台）。用 sovena_job_status 查询。"


@mcp.tool
def sovena_adhoc_list() -> str:
    ...
    if not packs:
        return "暂无资料包。用 sovena_adhoc_process 创建。"
    return "\n".join(
...
def sovena_doctor() -> str:
    ...
    try:
        ...
        probe = os.path.join(root, ".sovena_probe")
